...
class PropertyAvailabilityHelper {
     ...
     */
    static calculateAvailabilityInfo(property, reservations) {
        const now = new Date();
        // Filter active reservations (confirmed or pending, not cancelled or completed)
        const activeReservations = reservations.filter((res) => res.propertyId === property.id &&
            (res.status === Reservation_1.ReservationStatus.CONFIRMED ||
                res.status === Reservation_1.ReservationStatus.PENDING) &&
            new Date(res.endDate) >= now);
        // Sort by check-in date to find current and next bookings
        activeReservations.sort((a, b) => new Date(a.date).getTime() - new Date(b.date).getTime());
        // Find current booking (if any)
        const currentBooking = activeReservations.find((res) => new Date(res.date) <= now &&
            new Date(res.endDate) >= now);
        // Find next booking after current one
        const nextBooking = activeReservations.find((res) => new Date(res.date) > now ||
        ...
        if (currentBooking) {
            ...
            if (nextBooking && nextBooking !== currentBooking) {
                ...
                if (nextCheckOut > checkOutDate) {
                    ...
                    availabilityInfo.nextAvailableDate = new Date(checkOutDate.getTime() + 24 * 60 * 60 * 1000);
                }
            }
            else {
     ...
     */
    static createAvailabilityDisplay(property, availabilityInfo) {
        let message;
     ...
     */
    static getPropertiesAvailability(properties, reservations) {
        return properties.map((property) => {
            const propertyReservations = reservations.filter((res) => res.propertyId === property.id);
            const availabilityInfo = this.calculateAvailabilityInfo(property, propertyReservations);
            return this.createAvailabilityDisplay(property, availabilityInfo);
        });
    }
}
exports.PropertyAvailabilityHelper = PropertyAvailabilityHelper;
//# sourceMappingURL=PropertyAvailabilityHelper.js.map