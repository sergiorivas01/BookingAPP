...
const PropertyAvailabilityHelper_1 = require("../PropertyAvailabilityHelper");
const Property_1 = require("../Property");
const Reservation_1 = require("../../models/Reservation");
describe('PropertyAvailabilityHelper', () => {
    const createMockProperty = (id, name, availability = Property_1.AvailabilityStatus.AVAILABLE) => ({
        id,
        name,
    ...
    const createMockReservation = (id, propertyId, checkInDate, checkOutDate, status = Reservation_1.ReservationStatus.CONFIRMED) => ({
        id,
        clientId: 'client-1',
        propertyId,
        date: checkInDate,
    ...
    describe('calculateAvailabilityInfo', () => {
        it('should return available when property has no reservations', () => {
            ...
            const reservations = [];
            const result = PropertyAvailabilityHelper_1.PropertyAvailabilityHelper.calculateAvailabilityInfo(property, reservations);
        ...
        it('should return unavailable when property is in maintenance', () => {
            const property = createMockProperty('prop-1', 'Test Property', Property_1.AvailabilityStatus.MAINTENANCE);
            const reservations = [];
            const result = PropertyAvailabilityHelper_1.PropertyAvailabilityHelper.calculateAvailabilityInfo(property, reservations);
        ...
        it('should detect current booking when reservation is active', () => {
            ...
            const reservation = createMockReservation('res-1', 'prop-1', checkIn, checkOut, Reservation_1.ReservationStatus.CONFIRMED);
            const result = PropertyAvailabilityHelper_1.PropertyAvailabilityHelper.calculateAvailabilityInfo(property, [reservation]);
            expect(result.isAvailable).toBe(false);
        ...
        it('should ignore cancelled reservations', () => {
            ...
            const cancelledReservation = createMockReservation('res-1', 'prop-1', checkIn, checkOut, Reservation_1.ReservationStatus.CANCELLED);
            const result = PropertyAvailabilityHelper_1.PropertyAvailabilityHelper.calculateAvailabilityInfo(property, [cancelledReservation]);
            expect(result.isAvailable).toBe(true);
            expect(result.currentBooking).toBeUndefined();
        });
        it('should ignore completed reservations', () => {
            ...
            const completedReservation = createMockReservation('res-1', 'prop-1', checkIn, checkOut, Reservation_1.ReservationStatus.COMPLETED);
            const result = PropertyAvailabilityHelper_1.PropertyAvailabilityHelper.calculateAvailabilityInfo(property, [completedReservation]);
            expect(result.isAvailable).toBe(true);
            expect(result.currentBooking).toBeUndefined();
        });
        it('should ignore reservations for other properties', () => {
            ...
            const checkIn = new Date(now.getTime() - 1 * 24 * 60 * 60 * 1000);
            const checkOut = new Date(now.getTime() + 2 * 24 * 60 * 60 * 1000);
            const otherPropertyReservation = createMockReservation('res-1', 'prop-2', // Different property
            checkIn, checkOut);
            const result = PropertyAvailabilityHelper_1.PropertyAvailabilityHelper.calculateAvailabilityInfo(property, [otherPropertyReservation]);
            expect(result.isAvailable).toBe(true);
            expect(result.currentBooking).toBeUndefined();
        });
        it('should handle future booking correctly', () => {
            ...
            const futureReservation = createMockReservation('res-1', 'prop-1', checkIn, checkOut);
            const result = PropertyAvailabilityHelper_1.PropertyAvailabilityHelper.calculateAvailabilityInfo(property, [futureReservation]);
            expect(result.isAvailable).toBe(true);
            expect(result.currentBooking).toBeUndefined();
        ...
        it('should calculate next available date after current booking ends', () => {
            ...
            const reservation = createMockReservation('res-1', 'prop-1', checkIn, checkOut);
            const result = PropertyAvailabilityHelper_1.PropertyAvailabilityHelper.calculateAvailabilityInfo(property, [reservation]);
            expect(result.nextAvailableDate).toBeDefined();
            if (result.nextAvailableDate) {
                const nextAvailable = new Date(checkOut.getTime() + 24 * 60 * 60 * 1000);
                expect(result.nextAvailableDate.getTime()).toBeCloseTo(nextAvailable.getTime(), -3);
            }
        });
        it('should handle multiple reservations correctly', () => {
            ...
            const currentReservation = createMockReservation('res-1', 'prop-1', currentCheckIn, currentCheckOut);
            // Future booking
            const futureCheckIn = new Date(now.getTime() + 5 * 24 * 60 * 60 * 1000);
            const futureCheckOut = new Date(now.getTime() + 8 * 24 * 60 * 60 * 1000);
            const futureReservation = createMockReservation('res-2', 'prop-1', futureCheckIn, futureCheckOut);
            const result = PropertyAvailabilityHelper_1.PropertyAvailabilityHelper.calculateAvailabilityInfo(property, [currentReservation, futureReservation]);
            expect(result.isAvailable).toBe(false);
            expect(result.currentBooking?.reservationId).toBe('res-1');
    ...
    describe('createAvailabilityDisplay', () => {
        it('should create display for available property', () => {
            ...
            const display = PropertyAvailabilityHelper_1.PropertyAvailabilityHelper.createAvailabilityDisplay(property, availabilityInfo);
    ...
    describe('getPropertiesAvailability', () => {
        it('should return availability for multiple properties', () => {
            ...
            const reservation1 = createMockReservation('res-1', 'prop-1', new Date(now.getTime() - 1 * 24 * 60 * 60 * 1000), new Date(now.getTime() + 2 * 24 * 60 * 60 * 1000));
            // Property 2 has no bookings
            const reservations = [reservation1];
            const results = PropertyAvailabilityHelper_1.PropertyAvailabilityHelper.getPropertiesAvailability([property1, property2], reservations);
        ...
        it('should filter reservations by property correctly', () => {
            ...
            const now = new Date();
            const reservation1 = createMockReservation('res-1', 'prop-1', new Date(now.getTime() - 1 * 24 * 60 * 60 * 1000), new Date(now.getTime() + 2 * 24 * 60 * 60 * 1000));
            const reservation2 = createMockReservation('res-2', 'prop-2', new Date(now.getTime() + 5 * 24 * 60 * 60 * 1000), new Date(now.getTime() + 8 * 24 * 60 * 60 * 1000));
            const results = PropertyAvailabilityHelper_1.PropertyAvailabilityHelper.getPropertiesAvailability([property1, property2], [reservation1, reservation2]);
            expect(results[0].propertyId).toBe('prop-1');
            expect(results[0].isAvailable).toBe(false);
            expect(results[1].propertyId).toBe('prop-2');
            expect(results[1].isAvailable).toBe(true);
        ...
        it('should handle empty properties array', () => {
            const results = PropertyAvailabilityHelper_1.PropertyAvailabilityHelper.getPropertiesAvailability([], []);
            expect(results).toHaveLength(0);
        });
        it('should handle properties with no reservations', () => {
            const property1 = createMockProperty('prop-1', 'Property 1');
            const property2 = createMockProperty('prop-2', 'Property 2');
            const results = PropertyAvailabilityHelper_1.PropertyAvailabilityHelper.getPropertiesAvailability([property1, property2], []);
            expect(results).toHaveLength(2);
    ...
    describe('calculateDaysBetween (indirect testing)', () => {
        it('should calculate correct duration for bookings', () => {
            ...
            const currentReservation = createMockReservation('res-2', 'prop-1', currentCheckIn, currentCheckOut);
            const currentResult = PropertyAvailabilityHelper_1.PropertyAvailabilityHelper.calculateAvailabilityInfo(property, [currentReservation]);
...
//# sourceMappingURL=PropertyAvailabilityHelper.test.js.map