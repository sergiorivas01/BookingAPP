...
describe('ClientService', () => {
    ...
    beforeEach(() => {
        ...
        service = new ClientService_1.ClientService(mockStorage);
    });
    describe('createClient', () => {
        ...
        it('should throw error for invalid email format', async () => {
            ...
            await expect(service.createClient(invalidDTO)).rejects.toThrow('Invalid email format');
        ...
        it('should throw error if email already exists', async () => {
            ...
            await expect(service.createClient(validDTO)).rejects.toThrow('Client with this email already exists');
            expect(mockStorage.saveClient).not.toHaveBeenCalled();
        });
    ...
    describe('updateClient', () => {
        ...
        it('should update client with valid data', async () => {
            ...
            expect(updated.updatedAt.getTime()).toBeGreaterThan(existingClient.updatedAt.getTime());
            expect(mockStorage.updateClient).toHaveBeenCalled();
        });
        it('should throw error when client not found', async () => {
            mockStorage.getClient.mockResolvedValue(null);
            await expect(service.updateClient('non-existent', { name: 'New Name' })).rejects.toThrow('Client with id non-existent not found');
            expect(mockStorage.updateClient).not.toHaveBeenCalled();
        });
        it('should throw error for invalid email format', async () => {
            mockStorage.getClient.mockResolvedValue(existingClient);
            await expect(service.updateClient('client-1', { email: 'invalid-email' })).rejects.toThrow('Invalid email format');
            expect(mockStorage.updateClient).not.toHaveBeenCalled();
        });
        it('should throw error if new email already exists', async () => {
            ...
            await expect(service.updateClient('client-1', { email: 'other@example.com' })).rejects.toThrow('Client with this email already exists');
            expect(mockStorage.updateClient).not.toHaveBeenCalled();
        });
...
//# sourceMappingURL=ClientService.test.js.map