...
describe('ReservationService', () => {
    ...
    beforeEach(() => {
        ...
        service = new ReservationService_1.ReservationService(mockStorage);
    });
    describe('createReservation', () => {
        ...
        it('should throw error when client does not exist', async () => {
            ...
            await expect(service.createReservation(validDTO)).rejects.toThrow('Client with id client-1 not found');
        ...
        it('should throw error for past date', async () => {
            ...
            mockStorage.getClient.mockResolvedValue(mockClient);
            await expect(service.createReservation(dtoWithPastDate)).rejects.toThrow('Cannot create reservation for a past date');
            expect(mockStorage.saveReservation).not.toHaveBeenCalled();
        });
        it('should throw error for past end date', async () => {
            const pastEndDate = new Date();
            pastEndDate.setDate(pastEndDate.getDate() - 1);
            const dtoWithPastEndDate = {
                ...validDTO,
                endDate: pastEndDate,
            };
            mockStorage.getClient.mockResolvedValue(mockClient);
            await expect(service.createReservation(dtoWithPastEndDate)).rejects.toThrow('Cannot create reservation with end date in the past');
            expect(mockStorage.saveReservation).not.toHaveBeenCalled();
        });
        it('should throw error when end date is before start date', async () => {
            ...
            const dtoWithInvalidDates = {
                ...validDTO,
                date: startDate,
                endDate: endDate,
            };
            mockStorage.getClient.mockResolvedValue(mockClient);
            await expect(service.createReservation(dtoWithInvalidDates)).rejects.toThrow('End date must be after start date');
            expect(mockStorage.saveReservation).not.toHaveBeenCalled();
        });
        it('should throw error for zero guests', async () => {
            const dtoWithZeroGuests = {
                ...validDTO,
                numberOfGuests: 0,
            };
            mockStorage.getClient.mockResolvedValue(mockClient);
            await expect(service.createReservation(dtoWithZeroGuests)).rejects.toThrow('Number of guests must be greater than 0');
            expect(mockStorage.saveReservation).not.toHaveBeenCalled();
        });
        it('should throw error for negative guests', async () => {
            const dtoWithNegativeGuests = {
                ...validDTO,
                numberOfGuests: -1,
            };
            mockStorage.getClient.mockResolvedValue(mockClient);
            await expect(service.createReservation(dtoWithNegativeGuests)).rejects.toThrow('Number of guests must be greater than 0');
            expect(mockStorage.saveReservation).not.toHaveBeenCalled();
        });
    });
    ...
    describe('updateReservation', () => {
        ...
        it('should update reservation with valid data', async () => {
            ...
            expect(updated.updatedAt.getTime()).toBeGreaterThan(existingReservation.updatedAt.getTime());
            expect(mockStorage.updateReservation).toHaveBeenCalled();
        });
        it('should throw error when reservation not found', async () => {
            mockStorage.getReservation.mockResolvedValue(null);
            await expect(service.updateReservation('non-existent', { time: '20:00' })).rejects.toThrow('Reservation with id non-existent not found');
        ...
        it('should throw error for past date', async () => {
            ...
            pastDate.setDate(pastDate.getDate() - 1);
            mockStorage.getReservation.mockResolvedValue(existingReservation);
            await expect(service.updateReservation('reservation-1', { date: pastDate })).rejects.toThrow('Cannot update reservation to a past date');
            expect(mockStorage.updateReservation).not.toHaveBeenCalled();
        });
        it('should throw error for past end date', async () => {
            const pastEndDate = new Date();
            pastEndDate.setDate(pastEndDate.getDate() - 1);
            mockStorage.getReservation.mockResolvedValue(existingReservation);
            await expect(service.updateReservation('reservation-1', { endDate: pastEndDate })).rejects.toThrow('Cannot update reservation with end date in the past');
            expect(mockStorage.updateReservation).not.toHaveBeenCalled();
        });
        it('should throw error when end date is before start date in update', async () => {
            ...
            endDate.setDate(endDate.getDate() + 5); // Before start date
            mockStorage.getReservation.mockResolvedValue(existingReservation);
            await expect(service.updateReservation('reservation-1', {
                date: startDate,
                endDate: endDate,
        ...
        it('should throw error for zero guests', async () => {
            mockStorage.getReservation.mockResolvedValue(existingReservation);
            await expect(service.updateReservation('reservation-1', { numberOfGuests: 0 })).rejects.toThrow('Number of guests must be greater than 0');
            expect(mockStorage.updateReservation).not.toHaveBeenCalled();
        });
        it('should throw error for negative guests', async () => {
            mockStorage.getReservation.mockResolvedValue(existingReservation);
            await expect(service.updateReservation('reservation-1', { numberOfGuests: -1 })).rejects.toThrow('Number of guests must be greater than 0');
            expect(mockStorage.updateReservation).not.toHaveBeenCalled();
        });
        it('should allow updating status', async () => {
            ...
            expect(updated.status).toBe(Reservation_1.ReservationStatus.CONFIRMED);
            expect(mockStorage.updateReservation).toHaveBeenCalled();
        });
    });
    describe('deleteReservation', () => {
        ...
        it('should throw error when reservation not found', async () => {
            mockStorage.getReservation.mockResolvedValue(null);
            await expect(service.deleteReservation('non-existent')).rejects.toThrow('Reservation with id non-existent not found');
    ...
    describe('getReservationWithClient', () => {
        ...
        it('should throw error when reservation does not exist', async () => {
            mockStorage.getReservation.mockResolvedValue(null);
            await expect(service.getReservationWithClient('missing')).rejects.toThrow('Reservation with id missing not found');
        });
    });
    describe('getPropertyReservationsWithClients', () => {
        ...
        const reservationWithoutClient = {
        ...
        it('should return reservations along with their clients', async () => {
            ...
            mockStorage.getClient.mockImplementation(async (clientId) => {
                if (clientId === 'client-1') {
                    return mockClient;
                }
                return null;
            });
            const results = await service.getPropertyReservationsWithClients('property-1');
            ...
            expect(mockStorage.getReservationsByProperty).toHaveBeenCalledWith('property-1');
        });
    });
    describe('confirmReservation', () => {
        it('should confirm a reservation', async () => {
            ...
            expect(confirmed.status).toBe(Reservation_1.ReservationStatus.CONFIRMED);
            expect(mockStorage.updateReservation).toHaveBeenCalledWith('reservation-1', expect.objectContaining({ status: Reservation_1.ReservationStatus.CONFIRMED }));
        });
    });
    describe('cancelReservation', () => {
        it('should cancel a reservation', async () => {
            ...
            expect(cancelled.status).toBe(Reservation_1.ReservationStatus.CANCELLED);
            expect(mockStorage.updateReservation).toHaveBeenCalledWith('reservation-1', expect.objectContaining({ status: Reservation_1.ReservationStatus.CANCELLED }));
        });
    });
});
